    ...
    if 'Bk' in params['estimator']:
        ...
        for ell in params['multipoles']['Bk']:  
            ...
            if (params['k_fit']['Bk'][ell]==params['k_model']['Bk']).all():
                model_vect.extend(bk_evaluation[ell][(params['k_emul'][ell]>params['k_edges']['Bk'][ell][0]) & (params['k_emul'][ell]<params['k_edges']['Bk'][ell][1])])
            else:
                if 'k_emul' in params: keval = params['k_emul'][ell]
...
class PrepareFit(ComputePowerBiSpectrum):
    ...
    def set_bispectrum_model(self):
        ...
        if self.params['cosmo_inference']:
            ...
            z = self.params['z_eff']
            cache_path = os.path.expanduser('~')+f'/bicker_cache/z{z}/'
            for ell in self.params['multipoles']['Bk']: 
                ...
                for gp in group_to_emul:
                    emu[ell][gp] = BICKER.component_emulator(gp, ell, cache_path)
    ...
    def set_power_spectrum_model(self):
        ...
        if self.params['cosmo_inference']:
            z = self.params['z_eff']
            cache_path = f'/home/rneveux/bicker_cache/z{z}/'
            self.params['emu_pk'] = {ell: BICKER.power(ell, cache_path) for ell in self.multipoles['Pk']}
